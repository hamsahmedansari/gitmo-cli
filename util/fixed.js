...
const GitFixed = async msg => {
  ...
  try {
    ...
    console.log(chalk.yellowBright("adding Fixed commit to your repo"));

    let commitMsg = helper.createCommit(msg, ModifiedFiles, UntrackedFiles);

    await shell.exec(`git commit -m "Fixed: ${emoji.fixed} ${commitMsg} "`);
