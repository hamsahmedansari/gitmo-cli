...
const GitFormat = async msg => {
  ...
  try {
    ...
    console.log(chalk.yellowBright("adding Cosmetic commit to your repo"));

    let commitMsg = helper.createCommit(msg, ModifiedFiles, UntrackedFiles);

    await shell.exec(`git commit -m "Cosmetic: ${emoji.format} ${commitMsg} "`);
    ...
    console.log(chalk.bgGreenBright("Your current Dir is now Git Repo"));
    process.exit();
  } catch (error) {
    process.exit();
  }
