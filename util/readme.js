...
const GitAddReadMe = async msg => {
  ...
  try {
    console.log(chalk.yellowBright("Adding Readme.md in to your git"));
    await shell.exec("git add readme.md");
    console.log(chalk.greenBright("All Files Added "));
    console.log(chalk.yellowBright("adding Readme.md commit to your repo"));

    let commitMsg = helper.createCommit(msg, ModifiedFiles, UntrackedFiles);

    await shell.exec(`git commit -m "Update: ${emoji.readme} ${commitMsg} "`);
